...
#    result_lines << "--- , --- , --- , --- ,"
    ...
    (first_day..last_day).each do |date|
      ...
      if daily_report
        result_lines << "#{date_string(date)}, #{daily_report.daily_working_time.to_hyphen.join ' '} , #{daily_report.working_times_sum} ,#{daily_report.midnight_working_times_sum} ,"
      ...
      if date.saturday? || (date == date.end_of_month) #週次集計
        result_lines << ",週合計, #{HyphenTime.to_hyphen(weekly_working_min)} , #{HyphenTime.to_hyphen(weekly_midnight_working_min)} ,"
        weekly_working_min = 0
        weekly_midnight_working_min = 0
      end
    end

    result_lines << "月合計,, #{working_times_sum} , #{midnight_working_times_sum} ,"
